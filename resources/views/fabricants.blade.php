...
@section('content')
                ...
                <div class="card mb-4 mx-4">
                    <div class="card-header pb-0">
                        <div class="d-flex flex-row justify-content-between">
                            <div>
                                <h5 class="mb-0">Fabricants</h5>
                            </div>
                            <div>
                                <button type="button" href="#" class="btn bg-gradient-info btn-sm mb-0"
                                    data-bs-toggle="modal" data-bs-target="#ajouterModal">+&nbsp; Fabricant</button>
                            </div>
                            ...
                            <table class="table table-hover align-items-center mb-0 cell-border compact hover" data-page-length='6'
                                ...
                                <tbody class="fabricantBody">
                                    @foreach ($fabricants as $item)
                                        <tr>
                                            <td class="ps-4" onclick="window.location = '{{ route('tps', ['id' => $item->id]) }}' ;">
                                                <p class="text-xs font-weight-bold mb-0">{{ $item->id }}</p>
                                            </td>
                                            <td class="text-left" onclick="window.location = '{{ route('tps', ['id' => $item->id]) }}' ;">
                                                <p class="text-xs font-weight-bold mb-0">{{ $item->nom }}</p>
                                            </td>
                                            <td class="text-left" onclick="window.location = '{{ route('tps', ['id' => $item->id]) }}' ;">
                                                <p class="text-xs font-weight-bold mb-0">{{ $item->types->count() }}</p>
                                            </td>
                                            <td class="">
    ...
    @include('components.Modals.Fabricants.add')
    @include('components.Modals.Fabricants.edit')
    @include('components.Modals.Fabricants.delete')
@endsection
...
@section('scripts')
    ...
    <script src="{{ asset('js/Fabricants/edit.js') }}"></script>
@endsection
