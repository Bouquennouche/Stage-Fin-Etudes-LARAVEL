...
@section('content')
                ...
                <div class="card mb-4 mx-4">
                    <div class="card-header pb-0">
                        <div class="d-flex flex-row justify-content-between">
                            <div>
                                <h5 class="mb-0">Types : {{ $fabricant->nom}}</h5>
                            </div>
                            <div>
                                <button type="button" href="#" class="btn bg-gradient-info btn-sm mb-0"
                                    data-bs-toggle="modal" data-bs-target="#ajouterModal">+&nbsp; Type</button>
                            </div>
                            ...
                            <table class="table table-hover align-items-center mb-0 cell-border compact hover" data-page-length='6'
                                ...
                                <tbody class="fabricantBody">
                                    @foreach ($types as $item)
                                        <tr>
                                            <td class="ps-4" onclick="window.location = '{{ route('refs', ['id' => $item->id]) }}' ;">
                                                <p class="text-xs font-weight-bold mb-0">{{ $item->id }}</p>
                                            </td>
                                            <td class="text-left" onclick="window.location = '{{ route('refs', ['id' => $item->id]) }}' ;">
                                                <p class="text-xs font-weight-bold mb-0">{{ $item->nom }}</p>
                                            </td>
                                            <td class="text-left" onclick="window.location = '{{ route('refs', ['id' => $item->id]) }}' ;">
                                                <p class="text-xs font-weight-bold mb-0">{{ $item->references->count() }}</p>
                                            </td>
                                            <td class="">
    ...
    @include('components.Modals.Types.add')
    @include('components.Modals.Types.edit')
    @include('components.Modals.Types.delete')
@endsection
...
@section('scripts')
    ...
    <script src="{{ asset('js/Types/edit.js') }}"></script>
@endsection
